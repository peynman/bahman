...
func (r *RepositoryManager) Query(entity services.Entity) services.QueryBuilder {
	...
	if ok {
		db = r.client.connections[conn.ConnectionName()]
		if db != nil {
	...
	if db == nil {
		db = r.client.connections["app"].Table(entity.TableName())
	}
...
func NewRepository(logger services.Logger, client *RDBMSClients) *RepositoryManager {
	r := new(RepositoryManager)
	r.client = client
	r.log = logger
	return r
}
